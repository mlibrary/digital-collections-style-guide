          ...
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="$display-layout='outline'">
                ...
                <qui:debug name="{name(.)}"><xsl:value-of select="$display-layout" /></qui:debug>
                <xsl:apply-templates select="*[starts-with(name(),'DIV')][not(name()=name(.))]" mode="ItemDetails"/>
              </xsl:when>           
...
<xsl:if test="PageNumber='NA'"><xsl:text>[unnumbered]</xsl:text></xsl:if>
  ...
  <xsl:template match="SummaryString">
    ...
    <xsl:for-each select="HitTerm"><xsl:text>&quot;</xsl:text><xsl:value-of select="."/><xsl:text>&quot; </xsl:text></xsl:for-each>
  </xsl:template>
