      ...
      <details class="panel" style="padding-bottom: 0">
        ...
        <div>
          <p style="font-size: 0.875rem">
            ...
            Follow the collection link <span class="material-icons" aria-hidden="true" style="vertial-align: bottom">link</span> to search that 
  ...
  <xsl:template match="qui:fieldset[@slot='collids']/qui:option">
    <li>
      <div class="[ flex ][ flex-gap-0_5 ]" style="align-items: center; padding: 0.25rem; border: 1px solid var(--color-blue-200); margin-bottom: 0.5rem">
        <input type="checkbox" name="c" value="{@value}" id="c-{position()}">
          <xsl:if test="@checked='checked'">
            <xsl:attribute name="checked">checked</xsl:attribute>
          </xsl:if>
        </input>
        <label for="c-{position()}" style="display: flex; gap: 0.25rem; flex-grow: 1; justify-content: space-between; align-items: center">
          <xsl:value-of select="." />
          <a href="{@data-href}" style="display: block">
            <span class="visually-hidden">Open the <xsl:value-of select="." /> Collection</span>
            <span class="material-icons" aria-hiden="true" style="font-size: 2.5rem">link</span>
          </a>
        </label>
      </div>
    </li>
