...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml"
  ...
  xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:exsl="http://exslt.org/common"
  ...
  <xsl:template match="qui:main">
    ...
    <div
      class="[ flex flex-flow-row flex-gap-1 ]">
  ...
  <xsl:template match="qui:main" mode="modal">
    ...
    <xsl:apply-templates
      select="qui:block[@slot='access-information']" />
    <xsl:apply-templates
      select="qui:block[@slot='login-information']" mode="copy-guts" />
    <xsl:call-template
      name="build-modal-controls" />
  </xsl:template>
  ...
  <xsl:template name="build-modal-controls">
    <div class="modal--footer">
      <xsl:if test="//qui:block[@slot='login-information']">
        <a class="button button--ghost no-underline"
          href="/cgi/i/image/image-idx?page=feedback&amp;to=content">Inquire about access</a>
      </xsl:if>
      <a class="button button--primary no-underline" href="/cgi/i/image/image-idx">Go to Digital
    Collections</a>
    </div>
  </xsl:template>
